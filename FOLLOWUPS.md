...
- [ ] [added 2026-06-15 02:10 UTC] **[CI `secrets` job fails fast — pre-existing, not P0]** The `secrets` job (`gitleaks/gitleaks-action@v2`, `.github/workflows/ci.yml`) fails in ~4s on PRs #2 and #3 — and was ALREADY failing on PR #2 before any P0/fmt change, so it is NOT a regression from this work. A 4-second fail = a setup/license/config error (e.g. gitleaks-action's org-license check or event handling), not a deep-history scan finding. — why deferred: separate from the P0 task; needs a deliberate triage call (add a `GITLEAKS_LICENSE`/`.gitleaksignore`, pin/replace the action, or confirm it's a real but benign test-key finding). Resume: open a failed `secrets` job log once a run completes (`gh run view --log-failed --job <id>`) and decide. Branch protection is not yet enabled (WIRING W1), so this is visible-but-not-merge-blocking today.
