...
- [x] [added 2026-07-08 04:40 UTC] retrieval-as-partial (offer a ResolvedPartial's cleared set as a partial-fix step) · closed 2026-07-08 05:40 UTC → built (this arc): `MappingKind::{Full,Partial{cleared}}`, `fix_mappings` accumulates partial mappings keyed by (plan, cleared-set), consumers label them honestly. See docs/corpus-lifecycle-design.md §1.
- [ ] [added 2026-07-08 05:40 UTC] Reopen-interaction for PARTIAL mappings — `fix_mappings` nets `Reopened` events against FULL mappings only; a partial mapping's confirmations do not yet net against reopens (a reopen of the whole ticket does not cleanly negate a narrower per-symptom clear). Decide the correct accounting (per-symptom reopen? cleared-set-scoped?) — why deferred: needs a semantic model for "a partial regressed"; resume in `corpus-client/store.rs` fix_mappings + `docs/corpus-lifecycle-design.md` §1.
- [ ] [added 2026-07-08 05:40 UTC] Per-machine CONFIG LEDGER — the config-transition primitive (design §2) takes `prior`+`current` inventories as inputs; a ledger that STORES each machine's inventory history would let a transition be detected autonomously (the 5070→5080 case without a tech supplying both). — why deferred: a storage subsystem of its own; the primitive is useful immediately and composes with it. Resume: `docs/corpus-lifecycle-design.md` §2.
