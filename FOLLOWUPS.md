...
- [ ] [added 2026-06-15 03:55 UTC] **[Phase 1–2 of the leak methodology remain]** Phase 0 (validating mints + biting poison harness, closes C1) is DONE on `feat/corpus-leak-prevention` (`cf95d1c`). Phase 1 (type split `StoredPlan`/`StoredSymptom` + remove `Serialize` from raw `Plan`/`Candidate`/`Outcome`/`DiagnosticEvent` + `Prose` leaf typing + sealed `Debug` + private `Contribution` fields + `trybuild` + write-gate idempotence) and Phase 2 (read-side `from_served` re-de-id + frozen stop-code/module dictionaries + ban `serde_json::Value` on boundary types) are the C3/C4/C5 hard stops — a LARGE workspace-wide serde refactor. — why deferred from this turn: Phase 0 is the verified foundation + closes the CRITICAL; Phases 1–2 are the next (big) increment. Resume on `feat/corpus-leak-prevention` per `docs/corpus-leak-prevention.md` §4.
