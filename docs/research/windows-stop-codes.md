...
<!-- Extracted verbatim from the official table; two markdown-fetch artifacts corrected (0xDF IMPERSONATING_WORKER_THREAD, 0x1D0 ACPI_FIRMWARE_WATCHDOG_TIMEOUT). Glance-check advised. -->
# Windows bug-check (stop) codes — complete reference

**379 codes.** Complete enumeration from Microsoft's official *Bug Check Code Reference* (the canonical list WinDbg `!analyze` keys on), verbatim code→name. The symbolic name IS the meaning (e.g. `DRIVER_POWER_STATE_FAILURE`); for per-code parameter/cause detail each code has its own Microsoft Learn page (`bug-check-0x<code>--<name>`). Source: <https://learn.microsoft.com/en-us/windows-hardware/drivers/debugger/bug-check-code-reference2> (updated 2025-07-24).

| Hex code | Symbolic name |
|---|---|
