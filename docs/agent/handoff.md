# Agent handoff — 2026-07-02 18:56:49Z
...
HEAD:    4cf9d8f feat(serve): drop the `source` membership label from cec-diagnose/v1 (leak-C10)
status:  8 uncommitted path(s)
```

### Recent commits
```
4cf9d8f feat(serve): drop the `source` membership label from cec-diagnose/v1 (leak-C10)
1d2606d docs: trusted corpus access trajectory — B4/F1/F2/F3 sequence + decision-2 mapping
775113d docs(tracking): API-posture decisions — status, followups, handoff
878fd4d docs: corpus-over-API decision log; egress-sink checklist as binding policy
64ffa48 feat(serve): AGPL §13 network-service notice on --allow-remote
588f1ec test(serve): freeze the route surface; state the never-routable invariant
697e16d feat(support-agent): refuse a non-loopback inference endpoint without --allow-remote-inference
...
```
 ...
 M HANDOFFS.md
 M TODOS.md
 M docs/api-extension-design.md
 M docs/corpus-leak-prevention.md
 M docs/integration-rfc-for-chris.md
?? docs/corpus-cartography-threat.md
```
