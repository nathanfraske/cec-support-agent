# Agent handoff — 2026-07-04 19:40:25Z
...
HEAD:    49314d0 docs: owner decisions — Q1 fully decided (separate keys) + D3 integration posture
status:  0 uncommitted path(s)
```

### Recent commits
```
49314d0 docs: owner decisions — Q1 fully decided (separate keys) + D3 integration posture
72985c6 docs: record owner decision RFC Q6 (provenance-graph minimization)
eed089f docs(handoffs): PR #17 open; baton points at merge-then-Phase-3
315853b docs(tracking): record the blind-panel outcome and fixes
8626f23 fix(common): blind-audit fixes — fingerprint domain separation + salt fail-closed hardening
1ebfaa7 docs: annotate the two remaining pre-migration unsalted-FNV mentions
748f65b docs+ops: record the v2 migration (BUILT notes, hook widened, tracking)
90ff2c2 feat(corpus-client): retrieval keys move from the GET URL into a POST body
e17f38f feat(common): keyed fingerprint under a per-deployment salt (leak-C7, cec-fingerprint-v2)
92df52d feat(corpus): serde-independent canonical chain encoding (cec-corpus-chain-v2)
