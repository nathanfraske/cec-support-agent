...
**Priority A–H knowledge panel is DONE and BANKED** (`docs/research/priority-knowledge-briefs.md` +
`priority-knowledge-audit.md`): 8 items, 104 verified findings, 23 dropped. Opus rigor trust grades:
A1 stop-codes HIGH, B1 collector HIGH, D2 privacy MED-HIGH, D3 license MED-HIGH, C1 reversal MEDIUM,
A2 event-IDs MEDIUM (Event 18 mislabel, some Q&A-sourced), E1 local-LLM MEDIUM (leads with a post-cutoff
single-blog figure), **D1 liability/consent LOW — wrong UCC citations, half dropped; must go to a real
attorney before any customer-facing contract.** Cross-cutting: good caveats but "confidence-in-summary,
caveat-in-footnote"; legal 2-source rule was satisfied by counting not corroboration.
